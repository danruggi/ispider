...
async def fetch(reqA, client, mod, conf):
    metadata = {}

    # UNPACKING the request
    url = reqA[0]
    request_discriminator = reqA[1]
    dom_tld = reqA[2]
    retries = reqA[3]
    depth = reqA[4]
    ...
    metadata['request_discriminator'] = request_discriminator
    ...
    metadata['connection_time'] = str(datetime.now())
    metadata['mod'] = mod;

    sub, dom, tld, path = get_url_parts(url)
    ...
    try:
        ...
        try:
            ...
            ppprint("ERROR FURL", e, 2)
            pass

        try:
            sub, dom, tld, path = get_url_parts(response_url)
            metadata['final_url_domain_tld'] = dom+"."+tld
        except Exception as e:
            ppprint("ERROR FURLTLD", e, 2)
            pass

        if metadata['final_url_domain_tld'].lower() != metadata['dom_tld'].lower():
            # ppprint("NO REDIRECTS", metadata['url'], '-->', metadata['final_url_domain_tld'], 2)
            metadata["status_code"] = -1
        ...
        if exclude_file_types_from_data(metadata['content']):
            raise Exception("Unsupported file type")

        if metadata['content'] is None:
            ppprint("CONTENT NONE")
            raise Exception("Bad content")

        if conf['MAX_DEPTH'] > 0 and metadata['depth'] > conf['MAX_DEPTH']:
        ...
        try:
            ...
            # ppprint("Error getting socket informations", url, 2)
            pass
        ...
        try:
            ...
            # ppprint("Error getting SSL informations", url, 2)
            pass

        try:
            metadata['num_bytes_downloaded'] = response.stream._response.num_bytes_downloaded
        except:
            # ppprint("Error getting Bytes Downloaded", url, 2)
            pass
...
async def async_main(conf, reqsA, proxy=None, mod=0, headers={}):
    """
    conf: required MAX_DEPTH
    reqsA: -- to be updated
        url = reqA[0]
        request_discriminator = reqA[1]
        dom_tld = reqA[2]
    ...
    # consider transport=transport
    async with httpx.AsyncClient(timeout=timeout, proxies=proxy, limits=limits, follow_redirects=True, headers=headers) as client:
        try:
            tasks = [asyncio.create_task(fetch(reqA, client, mod, conf)) for reqA in reqsA]
